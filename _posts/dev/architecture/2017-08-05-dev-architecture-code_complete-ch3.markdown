...
프로젝트가 성공할 것이냐 아니냐의 상당 부분은 구현 시작 전에 이미 결정된다.

**구현 작업은 프로젝트 기간 중 30~65%를 차지하는 비싼 작업**이고, 준비 작업을 완전히 마치지 않은 상태에서는 2~3회 반복될 수 밖에 없다. **가장 비용이 많이 드는 구현 작업은 두번 이상해서는 안된다**.

## 선행 조건의 중요성

프로젝트에서는 각 단계별로 품질을 측정할 수 있다.

* 프로젝트의 마지막 단계 : 시스템 테스팅을 통하여 품질을 측정한다. 그러나 시스템 테스팅은 엉뚱한 제품을 개발하거나 목표에 일치하지 않는 제품을 개발한 것과 같은 문제점을 발견할 수 없다.
* 프로젝트의 중간 단계 : 구현 방법을 통한 품질을 측정한다.
* 프로젝트의 시작 단계 : 요구사항을 수집하고 설계하고 계획하므로써 측정한다.

구현 작업에서 프로젝트의 실패가 예측된다면, 다시 시작 단계로 돌아가서 요구사항과 설계를 검토할 필요가 있다. 엉뚱한 소프트웨어를 개발하는 것보다 일정이 지체된다고 해도 요구사항을 만족하는 소프트웨어를 개발하는 것이 낫다.
...
개발자들도 선행 작업의 중요성에 대해 알고 있고 선행 작업을 충분히 수행한다고 하지만, 실제로는 그렇지 않다.
...
반복적인 접근 방법에서는 선행 조건이 그다지 중요하지 않다고 주장하는 사람들도 있지만, 반복적인 접근 방법은 잘못된 성핸 작업의 영향을 줄여줄 수는 있어도 완전히 제거하지는 못한다.

순차적인 접근 방법은 선행 작업->구현->테스트->인계 과정을 전체 프로젝트 라이프 사이클에서 1회 실시한다.
반면에 반복적인 접근 방법은 위 과정을 프로젝트 라이프 사이클에서 1회 이상 실시한다.

반복적인 접근 방법은 순차적인 접근 방법에서 1회 실시하는 선행작업이 잘못되었을 때를 각 반복 작업에서 파악할 수 있어 상대적으로 위험을 줄여줄 뿐이지, 아예 선행 작업이 필요하지 않은 것은 아니다.

선행 작업을 진행할 당시에 프로젝트의 모든 문제점을 알고 있는 것이 아니기 때문에 해당 작업이 완벽할 수는 없다.
선행 작업이 완벽할 수 없다는 것을 인식하고, 추후에 부족한 부분을 개선해 나갈 수 있도록 하는 것이 중요하다.
